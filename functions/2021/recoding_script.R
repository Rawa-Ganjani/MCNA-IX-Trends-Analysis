recoding_2021 = function(df, loop) {
  
  df$received_aid <- case_when(df$aid_received == "yes" ~ 1,
                               is.na(df$aid_received) ~ NA_real_,
                               TRUE ~ 0)
  ...
  df$received_aid_other_nfi <- df$aid_type.other_nfis
  df$received_aid_protection <- df$aid_type.protection_legal_services
  df$received_aid_education <- df$aid_type.education_services
  df$received_aid_healthcare <- df$aid_type.healthcare
  df$received_aid_none <- df$aid_type.none
  
  df$aid_satisfaction <- case_when(df$aid_satisfaction == "yes" ~ 1,
  ...
  df$shelter.shelter_type <- df$shelter_type_area_assessed
    ...
    loop %>% mutate(
      ...
      disabled = case_when(
        ...
        difficulty_communicating %in% c("a_lot_of_difficulty", "cannot_do_at_all") |
          ...
          difficulty_selfcare %in% c("a_lot_of_difficulty", "cannot_do_at_all") ~ 1,
        TRUE ~ 0
      ),
      not_attending_school = case_when(
        school_regular_attendance_formal == "no" ~ 1,
        TRUE ~ 0
      ),
      female_not_attending_school = case_when(
        school_regular_attendance_formal == "no" &
          sex == "female" ~ 1,
        TRUE ~ 0
      ),
      male_not_attending_school = case_when(
        school_regular_attendance_formal == "no" &
          sex == "male" ~ 1,
        TRUE ~ 0
      ),
      ...
      unemployed_seek_work = case_when(
        age < 18 ~ NA_real_,
        work == "no" &
          actively_seek_work %in% c("yes", "no_opportunity") ~ 1,
        TRUE ~ 0
      ),
      female_unemployed_seek_work = case_when(
        age < 18 ~ NA_real_,
        work == "no" &
          actively_seek_work %in% c("yes", "no_opportunity") &
          sex == "female" ~ 1,
        TRUE ~ 0
      ),
  ...
  df <-
    left_join(df, sum_loop, by = c("X_uuid" = "X_submission__uuid"))
  ...
  df$education_barriers_not_function                     <- df$reasons_not_attend.school_not_functioning
  df$education_barriers_safety                           <- df$reasons_not_attend.school_not_safe
  df$education_barriers_cost                             <- df$reasons_not_attend.cannot_affort_school_expenses
  df$education_barriers_unable_to_register               <- df$reasons_not_attend.not_able_to_register_school
  df$education_barriers_health_issue                     <- df$reasons_not_attend.health_condition_child
  df$education_barriers_physical_limitation              <- df$reasons_not_attend.limited_access_school
  df$education_barriers_overcrowded                      <- df$reasons_not_attend.school_overcrowded
  df$education_barriers_poor_infrastructure              <- df$reasons_not_attend.school_infrastructure_poor
  df$education_barriers_curriculum_not_adapted           <- df$reasons_not_attend.curriculum_not_adapted
  df$education_barriers_parental_refusal                 <- df$reasons_not_attend.parental_refusal
  df$education_barriers_working                          <- df$reasons_not_attend.children_working
  df$education_barriers_lack_interest                    <- df$reasons_not_attend.lack_interest_education
  df$education_barriers_distance_learning_lack_resources <- df$reasons_not_attend.no_access_distance_learning_no_required_resources
  df$education_barriers_distance_learning_no_alternative <- df$reasons_not_attend.no_access_distance_learning_no_alternative_education
  df$education_barriers_none                             <- df$reasons_not_attend.none
  ...
  df$hospital_access_services <-
    case_when(
      df$distance_hospital <= 60 &
        df$hospital_emergency_ser == "yes" &
        df$hospital_maternity_ser == "yes" &
        df$hospital_pediatric_ser == "yes" &
        df$hospital_surgical_ser == "yes" ~ 1,
      T ~ 0
    )
  
  df$clinic_access <- case_when(df$distance_clinic <= 60 ~ 1, T ~ 0)
  
  df$difficulty_access_healthcare <-
    case_when(
      (df$difficulty_types_accessing_services.high_costs == 1 |
        df$difficulty_types_accessing_services.no_access_qualified_staff == 1 |
        df$difficulty_types_accessing_services.pb_civil_documents == 1 |
        df$difficulty_types_accessing_services.no_referral == 1 |
        df$difficulty_types_accessing_services.not_open == 1 |
        df$difficulty_types_accessing_services.long_distance_transportation_constraints == 1 |
        df$difficulty_types_accessing_services.medical_staff_refused_treament == 1 |
        df$difficulty_types_accessing_services.no_medicine_available == 1 |
        df$difficulty_types_accessing_services.no_treatment_my_disability == 1 |
        df$difficulty_types_accessing_services.not_inclusive_disabilities == 1 |
        df$difficulty_types_accessing_services.insufficient_nb_female_staff == 1 |
        df$difficulty_types_accessing_services.fear_covid == 1 |
        df$difficulty_types_accessing_services.fear_distrust == 1 |
        df$difficulty_types_accessing_services.waiting_time_too_long == 1) &
      df$health_accessed_number > 0 ~ 1,
      is.na(df$health_accessed_number) ~ NA_real_,
      T ~ 0
    )
  ...
  df$difficulty_accessing_female_services <-
    case_when(
      df$health_barriers.no_services == 1 |
        df$health_barriers.financial_constraints == 1 |
        df$health_barriers.transport_distance == 1 |
        df$health_barriers.lack_documentation == 1 |
        df$health_barriers.unappropriate_services == 1 |
  ...
  df$no_movement_12 <-
    case_when(
      df$movement_intentions12 == "current" ~ 1,
  ...
  df$return_security_aod <-
    case_when(
      df$reason_to_return_to_aoo.no_safe_aod == 1 |
        ...
        df$reason_to_return_to_aoo.lack_safety_security_women_girls == 1 |
  ...
  df$no_return_security_aoo  <-
    case_when(
      df$why_not_return.discrimination == 1 |
        ...
        df$why_not_return.community_tension == 1 |
  ...
  df$no_return_lack_docs  <- df$why_not_return.civil_doc_required
    ...
    df$drinking_water_source %in%
      ...
      (
        df$drinking_water_source == "unimproved_water_trucking" &
          df$water_trucking_reason == "personal_preference"
      ) ~ "improved",
    TRUE ~ "unimproved"
  )
  ...
  df$improved_sanitation <- case_when(df$latrines %in% c("vip_pit",
                                                         "flush",
                                                         "pit_slab") ~ 1,
                                      TRUE ~ 0)
  ...
  df$problem_water_quality <-
    case_when(
      df$problems_water_quality.not_clear == 1 |
        df$problems_water_quality.taste == 1 |
        df$problems_water_quality.smell == 1 |
        df$problems_water_quality.other == 1 ~ 1,
      TRUE ~ 0
    )
